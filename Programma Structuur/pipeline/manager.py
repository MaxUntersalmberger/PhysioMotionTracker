...
class MocapPipeline:
    ...
    def process(self, frames: dict[str, FramePacket], run_detection: bool = True) -> PipelineResult:
        ...
        debug = PipelineDebugInfo(
            ...
            per_joint_view_count=tri_result.per_joint_view_count if tri_result is not None else {},
            per_joint_confidence=tri_result.per_joint_confidence if tri_result is not None else {},
            reconstruction_trust_score=tri_result.trust_score if tri_result is not None else 0.0,
            reconstruction_trust_state=tri_result.trust_state if tri_result is not None else "unavailable",
