...
class _CameraCardWidget(QFrame):
    ...
    def set_health(self, fps: float | None, dropped_count: int) -> None:
        ...
        self._health_label.setText(f"Health: {health} | {fps_text} | drops={dropped_count}")
...
class CameraGridWidget(QWidget):
    ...
    def set_sources(
        ...
        self._rebuild_cards()
        self.set_selected_source(previous_selection if previous_selection in self._cards else None)
    ...
    def update_batch(
        ...
        for source_id, card in self._cards.items():
            ...
            if frame is not None:
                self._update_fps(source_id, frame)
    ...
    def _update_fps(self, source_id: str, frame: FramePacket) -> None:
    ...
    def _refresh_summary(self) -> None:
        ...
        parts = [f"{len(self._sources)} camera(s)", f"selected={selected_text}"]
